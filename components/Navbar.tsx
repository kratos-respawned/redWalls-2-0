  ...
  return (
    <>
      ...
      <nav className="bg-[#121212] w-full max-w-screen-2xl mx-auto relative text-slate-200 flex justify-between items-center z-10 isolate ">
        ...
        <ul
          className={`flex flex-col border-0 md:w-max md:flex-row md:opacity-100 md:pointer-events-auto items-center bg-[#131313] w-full md:static text-white absolute bottom-0 left-0 ${
            menu
        ...
        >
          <NavLink name="Home" path={path} url="/" key={1} />
          <NavLink name="Anime" path={path} url="/Anime" key={2} />
          <NavLink name="Phone" path={path} url="/Phone" key={3} />
          <NavLink name="Wallpapers" path={path} url="/Walls" key={4} />
          <NavLink name="Widescreen" path={path} url="/WideScreen" key={5} />
...
const NavLink = ({ path, name, url }: NavProps) => {
  let active =
